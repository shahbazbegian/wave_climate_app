...
with tab_proj:
    ...
    if last_proj is not None:
        ...
        # IMPORTANT FIX: Filter and prepare data for plotting, preserving both scenarios
        filtered_results = []
        for df in projection_results:
            if df is not None and len(df) > 0:
                df_copy = df.copy()
                df_copy['time'] = pd.to_datetime(df_copy['time'])
                df_copy = df_copy[df_copy['time'] <= '2099-12-31']
                if len(df_copy) > 0:
                    filtered_results.append(df_copy)
        ...
        for df in filtered_results:
            if 'scenario' not in df.columns:
                # If scenario column doesn't exist, try to infer from data
                continue
            ...
            # Safety filter
            df_plot = df_plot[df_plot['time'] <= '2099-12-31']
            
            if len(df_plot) > 0:
                df_plot['year'] = df_plot['time'].dt.year
                yearly_avg = df_plot.groupby('year')[variable].mean().reset_index()
        ...
        ax.set_xlim([pd.Timestamp('2040-01-01'), pd.Timestamp('2099-12-31')])
        ...
        for df in filtered_results:
            ...
            if len(df_plot) > 0:
                df_plot['year'] = df_plot['time'].dt.year
                yearly_means = df_plot.groupby('year')[variable].mean()
                ...
                if len(x) > 1:  # Need at least 2 points for regression
                    ...
                    ax2.plot(x, y, 'o', color=color,
                             markersize=5, alpha=0.7, label=f'{label} - yearly')
                    ax2.plot(x, trend, '--', color=color, linewidth=2,
                              label=f'{label} trend (slope={slope:.4f} m/year)')

        # Set x-axis limit to show 2040-2100 range
