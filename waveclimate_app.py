...
DEFAULT_DATA_DIR = os.path.join(APP_DIR, "data", "mediterranean")

# Where to look for a zip archive containing the 6 default CSVs, in order of
# preference. Supports a few common places people drop the archive.
CANDIDATE_ZIP_PATHS = [
    os.path.join(APP_DIR, "data", "data.zip"),
    os.path.join(APP_DIR, "data", "mediterranean.zip"),
    os.path.join(APP_DIR, "data.zip"),
    os.path.join(DEFAULT_DATA_DIR, "data.zip"),
]

# Maps (kind, variable) -> base filename (without extension). Each base name
# may exist as a plain "<base>.csv" file, a gzip-compressed "<base>.csv.gz"
# file, or as a member inside one of CANDIDATE_ZIP_PATHS (matched by basename,
# ignoring any folder prefix inside the zip).
DEFAULT_FILES = {
...
def _find_zip_member(base):
    """Search CANDIDATE_ZIP_PATHS for a zip containing <base>.csv or
    <base>.csv.gz at any depth. Returns (zip_path, member_name) or None."""
    import zipfile
    for zip_path in CANDIDATE_ZIP_PATHS:
...
def default_file_source(kind, variable):
    """Resolve where a default dataset actually lives. Returns one of:
    ...
    plain_path = os.path.join(DEFAULT_DATA_DIR, f"{base}.csv")
    gz_path = os.path.join(DEFAULT_DATA_DIR, f"{base}.csv.gz")
    if os.path.isfile(plain_path):
        return ('path', plain_path)
    if os.path.isfile(gz_path):
        return ('path', gz_path)
    in_zip = _find_zip_member(base)
...
def default_files_available():
    """Check which of the 6 expected default datasets exist anywhere (plain
    file, .gz file, or inside a zip). Returns {base_name: bool}."""
    status = {}
    for (kind, variable), base in DEFAULT_FILES.items():
        status[base] = default_file_source(kind, variable) is not None
    return status


@st.cache_data(show_spinner=False)
def load_default_dataset(kind, variable):
    """Load one of the bundled Mediterranean Sea datasets from the repo,
    using the same parsing/column-standardization logic as uploaded files.
    ...
    source = default_file_source(kind, variable)
    if source is None:
        base = DEFAULT_FILES[(kind, variable)]
        return None, (
            f"Default file not found: data/mediterranean/{base}.csv "
...
with tab_data:
    ...
    st.session_state.data_source = 'mediterranean' if source_choice.startswith("Mediterranean") else 'custom'

    # ------------------------------------------------------------------
    # Mediterranean Sea default dataset — load straight from the repo.
    # Accepts a plain .csv, a gzip-compressed .csv.gz, or a member inside a
    # data.zip archive — whichever is found first, per file.
    # ------------------------------------------------------------------
    if st.session_state.data_source == 'mediterranean':
        st.caption(f"Reading bundled files from `{os.path.relpath(DEFAULT_DATA_DIR, APP_DIR)}/` in the repo "
                   "— plain `.csv`, compressed `.csv.gz`, or a `data.zip` containing them all also works.")

        availability = default_files_available()
        missing = [label for (label), ok in availability.items() if not ok]

        if missing:
            st.error(
                "Some default dataset files are missing from the repo:\n\n"
                + "\n".join(f"- `{m}.csv` (or `.csv.gz`, or inside `data.zip`)" for m in missing)
                + "\n\nUpload them (see the README in `data/mediterranean/`), then reload this app."
            )
        else:
            st.success("All 6 default Mediterranean Sea files found.")

        if st.button("📥 Load Mediterranean Sea dataset", type="primary", key="btn_load_mediterranean",
            ...
            with st.spinner("Loading default dataset..."):
                for kind, variable, target_dict, target_key in load_plan:
                    df, warning = load_default_dataset(kind, variable)
        ...
        st.divider()
        st.markdown("##### 🧪 Test dataset (e.g., Brisbane format)")
        f = st.file_uploader("Test CSV", type="csv", key="up_test_med")
        if f is not None and st.button("Load Test Data", key="btn_test_med", type="primary"):
...
with tab_proj:
    st.caption(
        "Projects future wave conditions (through year 2100) by scaling baseline observations "
