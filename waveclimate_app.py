...
# Built-in validation/test datasets — single-file datasets (just Hs/Tm over a
# given period), distinct from the 6-file baseline+RCP regional datasets
# above. Each entry's path is checked as a plain .csv first, then .csv.gz.
TEST_DATASETS = {
    'brisbane': {
        'label': 'Brisbane (2024)',
        'base_path': os.path.join(APP_DIR, "data", "mediterranean", "brisbane"),
    },
}


def _resolve_test_dataset_path(key):
    """Find the on-disk path for a built-in test dataset, preferring a plain
    .csv, falling back to .csv.gz. Returns None if neither exists."""
    base_path = TEST_DATASETS[key]['base_path']
    plain_path = f"{base_path}.csv"
    gz_path = f"{base_path}.csv.gz"
...
def load_builtin_test_dataset(key):
    """Load one of the built-in validation/test datasets (e.g. Brisbane)
    using the same flexible column-naming logic as uploaded test files."""
    path = _resolve_test_dataset_path(key)
    if path is None:
        return None, f"Default test file not found: {TEST_DATASETS[key]['base_path']}.csv (or .csv.gz)"
    return analysis.load_test_data(path)
...
st.markdown(
    """
    <style>
    /* Force a light theme regardless of the user's OS/browser color scheme,
       and set every text color explicitly so nothing inherits an invisible
       (e.g. dark-on-dark) default from a dark system preference. */
    .stApp {
    ...
    [data-testid="stMetricValue"] { color: #0b5394 !important; }
    [data-testid="stMetricLabel"] { color: #444444 !important; }
    .stAlert, [data-testid="stAlertContainer"] { color: #1a1a1a !important; }
    /* Tabs */
    button[data-baseweb="tab"] { color: #333333 !important; }
    button[data-baseweb="tab"][aria-selected="true"] { color: #0b5394 !important; }
    </style>
    """,
...
with col_title:
    ...
    st.markdown(
        ...
        "(SWH) and mean wave period (Tm) — under the RCP 4.5 and RCP 8.5 climate change scenarios. "
        "Fit extreme-value (GPD) statistics to storm peaks, scale baseline observations onto future "
        "projection years, and visualize long-term trends. Use the built-in Mediterranean Sea "
        "dataset or upload your own."
    )
with col_version:
    st.markdown(
        "<div style='text-align:right; color:#666666; padding-top:14px;'>"
...
with tab_data:
    st.caption(
        "📁 **Data Loading** — load the baseline (historical) and RCP 4.5 / RCP 8.5 projection "
        "datasets that every other tab works from. Use the built-in Mediterranean Sea dataset, "
        "or upload your own CSVs for SWH and Tm."
    )
    st.markdown("### 🗺️ Choose your data source")

    source_choice = st.radio(
        "Data source",
        options=["Mediterranean Sea (default)", "Upload my own data"],
        horizontal=True,
    ...
    # ------------------------------------------------------------------
    # Built-in regional dataset (Mediterranean Sea) — load straight from
    # the repo. Accepts a plain .csv, a gzip-compressed .csv.gz, or a
    # member inside a data.zip archive — whichever is found
    # first, per file.
    # ------------------------------------------------------------------
    if st.session_state.data_source in REGIONS:
        ...
        st.divider()
        st.markdown("##### 🧪 Optional: test dataset (validation)")
        test_choice = st.radio(
            "Test dataset source",
            options=["Brisbane (2024) — built-in", "Upload my own"],
            horizontal=True,
            key=f"test_source_radio_{region}",
            label_visibility="collapsed",
        )

        if test_choice.startswith("Brisbane"):
            brisbane_path = _resolve_test_dataset_path('brisbane')
            if brisbane_path is None:
                st.error(
                    "Built-in Brisbane dataset not found in the repo. Expected "
                    "`data/mediterranean/brisbane.csv` or `data/mediterranean/brisbane.csv.gz`."
                )
            else:
                if st.button("📥 Load Brisbane (2024) dataset", key=f"btn_test_builtin_{region}", type="primary"):
                    df, error = load_builtin_test_dataset('brisbane')
                    if error:
                        st.error(error)
                    else:
                        st.session_state.test_data = df
                        st.success(f"Brisbane test data loaded: {len(df)} records from {df['time'].min()} to {df['time'].max()}")
        else:
            f = st.file_uploader("Test CSV", type="csv", key=f"up_test_{region}")
            if f is not None and st.button("Load Test Data", key=f"btn_test_{region}", type="primary"):
...
"""
        ...
        # ---------------- Test data ----------------
        with st.expander("🧪 Test Data (validation)"):
            test_choice_custom = st.radio(
                "Test dataset source",
                options=["Brisbane (2024) — built-in", "Upload my own"],
                horizontal=True,
                key="test_source_radio_custom",
                label_visibility="collapsed",
            )

            if test_choice_custom.startswith("Brisbane"):
                brisbane_path = _resolve_test_dataset_path('brisbane')
                if brisbane_path is None:
                    st.error(
                        "Built-in Brisbane dataset not found in the repo. Expected "
                    ...
                    if st.button("📥 Load Brisbane (2024) dataset", key="btn_test_builtin_custom", type="primary"):
                        df, error = load_builtin_test_dataset('brisbane')
                        if error:
                            st.error(error)
                        else:
...
with tab_gpd:
    st.caption(
        "🎯 **GPD Analysis** — identifies extreme wave events by fitting a Generalized Pareto "
        "Distribution to peaks above a chosen threshold, after declustering nearby peaks from the "
        "same storm. Shows the threshold and declustered peaks over time, plus the fitted GPD curve "
        "against the histogram of exceedances, with the shape (ξ) and scale (β) parameters."
    )
    ...
    if last is not None:
        ...
        props = dict(boxstyle='round', facecolor='#f0f0f0', edgecolor='#999999', alpha=0.9)
...
with tab_proj:
    st.caption(
        "🔄 **Projections** — generates future wave time series by mapping the chosen RCP scenario "
        "(2041–2100) onto the baseline data, year by year. Shows yearly-average projected values "
        "for SWH or Tm under RCP 4.5, RCP 8.5, or both, plus a long-term linear trend line with its "
        "slope, so you can see the rate of change per year."
    )
...
with tab_results:
    st.caption(
        "📊 **Results** — bundles everything generated in the other tabs (loaded datasets, GPD "
        "fit parameters, and projected series) into a single downloadable ZIP of CSV files, so you "
        "can keep or share the full output of your analysis session."
    )
