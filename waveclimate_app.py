...
# may exist on disk as either "<base>.csv" or a gzip-compressed "<base>.csv.gz"
# — pandas auto-detects gzip from the .gz extension, so no special-casing is
# needed once we've found the right path.
DEFAULT_FILES = {
...
def default_file_path(kind, variable):
    """Resolve the actual on-disk path for a default dataset, preferring a
    plain .csv if present, falling back to .csv.gz. Returns None if neither
    exists."""
...
def default_files_available():
    """Check which of the 6 expected default datasets exist on disk (as
    either .csv or .csv.gz). Returns {base_name: bool}."""
    status = {}
    for (kind, variable), base in DEFAULT_FILES.items():
        status[base] = default_file_path(kind, variable) is not None
    return status
...
def load_default_dataset(kind, variable):
    ...
    Cached so repeated tab switches don't re-read from disk every rerun."""
    path = default_file_path(kind, variable)
    if path is None:
        base = DEFAULT_FILES[(kind, variable)]
        return None, f"Default file not found: data/mediterranean/{base}.csv (or .csv.gz)"
    if kind == 'baseline':
        df, warning = analysis.load_baseline_csv(path, variable)
    else:
        df, warning = analysis.load_projection_csv(path, variable)
    return df, warning
...
with tab_data:
    ...
    # Accepts either a plain .csv or a gzip-compressed .csv.gz with the
    # same base name (pandas auto-detects gzip from the .gz extension).
    # ------------------------------------------------------------------
    if st.session_state.data_source == 'mediterranean':
        st.caption(f"Reading bundled files from `{os.path.relpath(DEFAULT_DATA_DIR, APP_DIR)}/` in the repo "
                   "(plain `.csv` or compressed `.csv.gz`, either works).")
        ...
        if missing:
            st.error(
                ...
                + "\n".join(f"- `data/mediterranean/{m}.csv` (or `.csv.gz`)" for m in missing)
                + "\n\nUpload them to that folder in GitHub, then reload this app."
            )
        else:
        ...
        st.divider()
        st.markdown("##### 🧪 Optional: test dataset (e.g., Brisbane format)")
